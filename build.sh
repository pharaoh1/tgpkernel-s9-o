...
echo "- Building vendor.tar.xz file ..."
tar -cf - vendor/ | xz -9 -c - > vendor.tar.xz
mv -f vendor.tar.xz $ZIPDIR/tgpkernel/files/vendor.tar.xz
rm -rf $ZIPDIR/tgpkernel/vendor
...
LOGFILE=$OUTPUT/build-s9.log
FUNC_BUILD_BOOTIMG
mv -f $WORKDIR/ramdisk/image-new.img $ZIPDIR/tgpkernel/kernels/boot-s9.img
MODEL=S9+
export KERNELTITLE=$KERNELNAME.$MODEL.$VERSION_NUMBER
LOGFILE=$OUTPUT/build-s9+.log
FUNC_BUILD_BOOTIMG
...
if [ $KEEP = "yes" ]; then
	...
	cp -f $ZIPDIR/tgpkernel/kernels/boot-s9.img $OUTPUT/boot-s9.img
	cp -f $ZIPDIR/tgpkernel/kernels/boot-s9+.img $OUTPUT/boot-s9+.img
fi
ZIP_DATE=`date +%Y%m%d`
